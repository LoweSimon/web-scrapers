...
async function getTechnicalPaint()   {
    try {
        let base_url = "https://www.goblingaming.co.uk/";

        const response = await fetch (`${base_url}collections/paints-citadel?filter.p.m.search.paint_range=Technical&sort_by=title-ascending`);
        ...
        fs.writeFile("../goblingaming-paint-data/citadel/citadel-technical-paint.json", itemsString, function(err, result) {
